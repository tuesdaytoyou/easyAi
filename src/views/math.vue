<template>
  ...
  >
    <div 
      class="toolbar-arrow"
      :style="{left:toolArrowLeft}"
    ></div>
    <div
      contenteditable="false"
      class="toolbar-primary"
      :style="{left:toolPrimaryLeft}"
    >
      <ul>
        <li v-for="(li, index) in primaryList"><img :src="li.src" class="panel_box"></li>
      </ul>
    </div>
    <div
      contenteditable="false"
...
.toolbar-primary ul, .toolbar-secondary ul{
  display: flex;
  justify-content: center;
  align-items: center;
  border-radius: 3px;
}
.toolbar-primary li, .toolbar-secondary li{
  width: 30px;
  height: 30px;
  color: white;
  cursor: pointer;
...
.toolbar-primary li:hover, .toolbar-secondary li:hover{
  background: #505050;
}
.panel_box{
  width: 20px;
